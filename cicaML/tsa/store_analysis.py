...
def get_quant_high_low_sales(time_serie):
    ...
    quant_low_sales = (time_serie.groupby(time_serie.index.weekday).apply(
        lambda df: get_quant_low_sales(df, k1=0.5)).sum())
...
def get_lost_sales(sales_serie, stock_serie):
    ...
    weekday_high_stock_stats = (sales_serie_high_stock.groupby(
        sales_serie_high_stock.index.weekday).agg(
    ...
    for idx, data, sales in zip(sales_low_stock_idx.weekday,
                                sales_low_stock_idx,
                                sales_serie_low_stock.values):
    ...
    ts = TimeSeries(index=new_serie['data'],
